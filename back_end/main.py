...
#             break
#     try:
#         if not flag and request.method != 'OPTIONS':  # 判断请求可否跳过
#             payload = security.jwt_decode(request.headers['token'])  # 解析token
...
#                 return response(status.HTTP_200_OK, message.NOT_ALLOWED)
#     except Exception:
#         return response(status.HTTP_401_UNAUTHORIZED,
#                         'not allowed')  # 各种异常，如token不存在，则返回401代码
#     return await call_next(request)  # 若请求可接受，则执行请求


if __name__ == '__main__':
    uvicorn.run("main:app", reload=True, host='192.168.1.4', port=10086)
