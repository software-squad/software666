...
def select_by_2(index1, value1, index2, value2):
    """选取 JOBS 中 index1=value1, index2=value2 的行"""
    db, cursor = database.connect_to_database()
...
def insert(item):
    """在 JOBS 中增加一行 item"""
    db, cursor = database.connect_to_database()
    sql = "INSERT INTO %s(JOBNAME,REMARK,DEPTID) VALUES('%s','%s','%s')" % (  # 插入相关数据的数据库语句
